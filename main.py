...
INGESTION_DIR = BASE / "ingestion-layer"
# The hyphenated folder is not a Python package. Resolve its modules before
# importing them, independently of the directory used to start the server.
...
def create_app(data_dir=None, ingestor=None):
    data_dir = Path(data_dir or os.environ.get("INGESTION_DATA_DIR", INGESTION_DIR / "data")).resolve()
    templates = Jinja2Templates(directory=str(INGESTION_DIR / "ui"))
    ...
    app.mount("/static", StaticFiles(directory=INGESTION_DIR / "ui"), name="static")
    ...
    async def error_page(request, exc):
        if request.url.path.startswith("/api/pipeline/"):
            return JSONResponse({"type": "error", "message": str(exc.detail)}, status_code=exc.status_code)
    ...
    async def upload(request: Request):
        ...
        try:
            async with request.form(max_files=MAX_FILES, max_fields=5) as form:
                ...
                if request.url.path == "/api/pipeline/match-upload":
                    upload_values += form.getlist("doc_file")
                files = [value for value in upload_values
    ...
    from pipeline.api import create_router

    app.include_router(create_router(data_dir, enqueue_upload=upload))
    return app
