...
def _parse_date(raw: str) -> str:
    ...
    try:
        ...
        diagnostic(f"[FlightFinder] ⚠️ Gemini date parse failed: {e}")
    ...
    # Last resort: today
    diagnostic(f"[FlightFinder] ⚠️ Could not parse date '{raw}' — using today.")
...
def _search_flights_browser(
    ...
    diagnostic(f"[FlightFinder] 🌐 Opening: {url}")
...
def _parse_flights_with_gemini(
    ...
    try:
        ...
        diagnostic(f"[FlightFinder] ⚠️ Gemini parse failed: {e}")
        return []
...
def _save_to_desktop(content: str, origin: str, destination: str) -> str:
    ...
    diagnostic(f"[FlightFinder] 💾 Saved: {filepath}")
    ...
    try:
        ...
        diagnostic(f"[FlightFinder] ⚠️ Could not open text editor: {e}")
