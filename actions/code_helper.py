...
def _take_screenshot() -> Path | None:
    try:
        ...
        diagnostic(f"[Code] 📸 Screenshot: {screenshot_path}")
        return screenshot_path
    except Exception as e:
        diagnostic(f"[Code] ⚠️ Screenshot failed: {e}")
        return None
...
def _detect_intent(description: str, file_path: str, code: str) -> str:
    ...
    if desc:
        try:
            ...
            diagnostic(f"[Code] Intent classification failed ({e}) — structural fallback")
...
def _build(description, language, output_path, args, timeout, speak=None, player=None) -> str:
    ...
    try:
        ...
        diagnostic(f"[Code] ✅ Written: {path}")
    except Exception as e:
    ...
    for attempt in range(1, MAX_BUILD_ATTEMPTS + 1):
        diagnostic(f"[Code] 🔄 Attempt {attempt}/{MAX_BUILD_ATTEMPTS}")
        if player:
        ...
        diagnostic(f"[Code] ⚠️ Error on attempt {attempt}, fixing...")
        if player:
...
Updated code:"""
    ...
    diagnostic(f"[Code] ✅ Edited: {file_path}")
...
Optimized code:"""
    ...
    diagnostic(f"[Code] ✅ Optimized: {save_path}")
...
def _screen_debug_action(description, file_path, player, speak=None) -> str:
    ...
    diagnostic("[Code] 📸 Capturing screen for debug...")
    ...
    if file_path:
        ...
        if err:
            diagnostic(f"[Code] ⚠️ Could not read file: {err}")
...
Be specific and actionable. If you see an error message, quote it exactly."""
        ...
        diagnostic(f"[Code] ✅ Screen analysis complete")
        ...
        if file_path and file_content:
            ...
            if code_match:
                ...
                diagnostic(f"[Code] ✅ Fixed code saved: {file_path}")
...
def code_helper(
    ...
    if action == "auto":
        ...
        diagnostic(f"[Code] 🤖 Auto-detected: {action}")
